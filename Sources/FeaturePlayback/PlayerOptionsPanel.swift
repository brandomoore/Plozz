...
struct PlayerOptionsPanel: View {
    ...
    /// Space for the entire panel, after the host removes its top/bottom insets.
    /// Measured from that player's viewport, never the device's screen bounds.
    ...
    /// before it clamps + scrolls. Style screens use the remaining host viewport.
    private static let panelBodyMaxHeight: CGFloat = 440

    static func bodyHeightLimit(maximumHeight: CGFloat, headerHeight: CGFloat, styleFamily: Bool) -> CGFloat {
        guard maximumHeight.isFinite, headerHeight.isFinite else { return 0 }
        let available = max(0, maximumHeight - max(0, headerHeight) - PlozzFrostedSurface.dividerHeight)
        return styleFamily ? available : min(panelBodyMaxHeight, available)
    }

    private var maximumBodyHeight: CGFloat {
        Self.bodyHeightLimit(maximumHeight: maximumHeight, headerHeight: headerHeight, styleFamily: styleEditing)
    }
    ...
    /// - Style screens are top-pinned and scroll at the host's viewport limit. The
    ...
    private func morphingPanel(for category: Category) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            frostAwareDivider
            morphingBody(category: category) { panelBodyContent(for: category) }
        }
    ...
    private func morphingBody<Content: View>(
        ...
        let maximumBodyHeight = self.maximumBodyHeight
        ...
        if bodyHeight > 0 {
            ...
            .scrollDisabled(bodyHeight <= maximumBodyHeight)
            .frame(
                height: min(bodyHeight, maximumBodyHeight),
                alignment: .top
            )
        } else {
            ...
            body
                .frame(maxHeight: maximumBodyHeight, alignment: .top)
                .fixedSize(horizontal: false, vertical: true)
                .clipped()
        }
    }
    ...
    static func headerTitle(for category: Category, subtitleScreen: SubtitleScreen) -> LocalizedStringResource {
        ...
        case .styleFileFormatting: return "File Formatting"
        }
    }
    ...
    private var subtitleBody: some View {
        ...
        case .download:
            ...
            SubtitleDownloadScreen(model: model, actions: actions, focus: $focus)
                .frame(minHeight: maximumBodyHeight, alignment: .top)
        case .sync: SubtitleSyncScreen(model: model, actions: actions, focus: $focus)
        case .style, .styleFont, .styleSystemFont, .styleOutline, .styleBackground, .styleDual, .styleFileFormatting:
            SubtitleStylePanel(
    ...
    private func openSubtitleScreen(_ screen: SubtitleScreen) {
        ...
        // was actually displayed at. Changing limits without clamping would snap the box to
        ...
        if !subtitleScreen.isStyleFamily && screen.isStyleFamily {
            ...
            withTransaction(snap) {
                bodyHeight = min(bodyHeight, maximumBodyHeight)
            }
        }
